...
export default function Index() {
  ...
  // console.log("dynamicProviders UI:", dynamicProviders);
...
// //     filterType 
// //   );
// //   // console.log("DynamicGames:", games.length);
...
useEffect(() => {
  ...
  // 🔥 JANGAN FETCH kalau provider belum ada
...
useEffect(() => {
  ...
  const loadMostPlay = async () => {
    await fetchPageData();
    const mostplay = getMostPlay();
    setDynamicGames(mostplay);
  };
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status) {
        ...
        localStorage.setItem("username", loginPhone);

        setIsLoggedIn(true);
        setUserName(loginPhone);
  ...
  const handleScroll = () => {
    ...
    if (
      ...
      setVisibleCount((prev) => {
        if (prev >= dynamicGames.length) return prev;
        return prev + 21;
      });
    }
...
//=============== End Popup ==============//
 ...
 useEffect(() => {
  ...
  const loadBanners = async () => {
    ...
    console.log("Banner API RAW:", bannerData);

    const formatted = bannerData.map((b: any, index: number) => ({
      id: b.id || index + 1,
      image: b.url_mobile || b.url || "",
    }));

    console.log("Banner FORMATTED:", formatted);
...
useEffect(() => {
  const loadSEO = async () => {
    ...
    // 🎯 TITLE
    document.title = seo.default_website_title || seo.website_name;

    // 🎯 DESCRIPTION
    const desc = seo.running_text || "";
    ...
    // 🎯 CANONICAL
    if (seo.custom_canonical_global) {
      ...
      canonical.setAttribute("href", seo.custom_canonical_global);
    }

    // 🎯 META PIXEL
    injectHTML("meta-pixel", seo.meta_pixel, "head");
    
    // 🎯 GLOBAL SCRIPT (HEAD)
    // injectScript("custom-script-global", seo.custom_script_global, "head");
    injectHTML("custom-script-global", seo.custom_script_global, "head");
    
    // 🎯 BODY SCRIPT
    injectHTML("custom-script-body", seo.custom_script_body_global, "body");
    
    // 🎯 LIVECHAT SCRIPT
    // injectScript("livechat-script", seo.script_livechat, "body");
    injectHTML("livechat-script", seo.script_livechat, "body");
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-slide-up" style={{ backgroundColor: "#F178A1" }}>
              ...
              <form onSubmit={handleSignIn} className="px-6 py-4 space-y-4">
                ...
                {/* Phone Number */}
                <div>
                  <label className="block text-sm font-bold text-blue-600 mb-2">{t("phone_number")}</label>
                  <input
                    type="text"
      ...
      <header className="sticky top-0 z-40 shadow-lg" style={{ background: "linear-gradient(90deg, #F178A1 0%, #FFC1DA 100%)" }}>
        ...
        <div className="md:hidden flex items-center justify-between px-2 py-1 gap-2 h-16">
          ...
          <div className="flex flex-col items-center gap-0 flex-1 min-w-0">
            <span className="text-white font-bold text-xs">ima00180</span>
          ...
          <div className="w-80 h-screen overflow-y-auto flex flex-col animate-slide-right" style={{ background: "linear-gradient(180deg, #F178A1 0%, #FFC1DA 100%)" }}>
            ...
            <div className="px-6 py-4 bg-white rounded-2xl mx-4 mb-4">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-pink-400 to-blue-400 flex items-center justify-center text-2xl">
                  👤
                </div>
                <div className="flex-1">
                  <p className="font-bold text-sm">ima00180</p>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-bold">
                <div className="text-center">
                  <p className="text-pink-500">Ranking</p>
                  <p className="text-gray-700">point</p>
                </div>
                <div className="text-center">
                  <p className="text-pink-500">2,027.00</p>
            ...
            {/* Promotion */}
            <div className="px-4 mb-4">
              <button className="w-full bg-white rounded-lg p-3 text-center hover:shadow-lg transition-all flex items-center justify-center gap-2 font-bold text-gray-700">
